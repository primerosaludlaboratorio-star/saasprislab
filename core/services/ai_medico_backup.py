...
try:
    ...
    logging.warning("google-generativeai no está instalado. Instalarlo con: pip install google-generativeai")
...
def configurar_gemini():
    ...
    Returns:
        bool: True si se configuró correctamente, False si hay error
    """
    ...
    try:
        ...
        if not api_key:
            logger.error("GOOGLE_API_KEY no está configurado en settings o variables de entorno")
            return False
...
def procesar_consulta_medica(audio_file) -> Dict[str, Any]:
    ...
    try:
        logger.info("Iniciando procesamiento de consulta médica")
...
FORMATO DE SALIDA (JSON):
{
    ...
    "signos_vitales": {
        "temperatura": 36.5,  // En °C, null si no se menciona
...
REGLAS CRÍTICAS:
1. Solo incluye información que se mencione explícitamente en el audio
2. Si un dato no se menciona, usa null
3. NO inventes información
4. Respeta el formato JSON exacto
5. Para presión arterial, usa formato "sistólica/diastólica" (ej: "120/80")
...
def procesar_resultados_lab(audio_file, lista_parametros: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    try:
        logger.info("Iniciando procesamiento de resultados de laboratorio")
        ...
        # Construir lista de parámetros para el prompt
        parametros_info = "\n".join([
            f"- {p['nombre']}: Keywords: {p['keywords']}"
            for p in lista_parametros
        ])
        
        # Prompt especializado para laboratorio
        prompt = f"""
Eres un asistente de laboratorio clínico experto en transcripción de resultados.

TAREA:
Escucha el audio donde se dictan valores de pruebas de laboratorio y mapéalos a los parámetros disponibles.
...
EJEMPLOS:
- Si se dice "Glucosa 98", devuelve {{"glucosa": 98}}
- Si se dice "La hemoglobina salió en 14.5", devuelve {{"hemoglobina": 14.5}}
- Si se dice "Ponle 5.4 a la glicada", devuelve {{"hba1c": 5.4}}

REGLAS CRÍTICAS:
1. Solo incluye valores que se mencionen explícitamente
2. Usa el nombre del parámetro en minúsculas y sin espacios ni acentos
3. Si se menciona una prueba pero no el valor, NO la incluyas
4. Usa punto decimal para decimales (ej: 14.5, no 14,5)
5. Si no estás seguro del mapeo, NO inventes

IMPORTANTE:
- Usa las keywords para identificar correctamente cada parámetro
- Si se dice "glicada" o "a1c", corresponde a "hba1c"
- Si se dice "azúcar" o "glicemia", corresponde a "glucosa"
...
"""
        ...
        logger.info(f"✓ Resultados de laboratorio procesados: {len(data)} valores")
        return data
        
    except Exception as e:
        logger.error(f"Error al procesar resultados de laboratorio: {e}", exc_info=True)
...
def test_gemini_connection():
    ...
    try:
        ...
        # Crear modelo y hacer una prueba simple
        model = genai.GenerativeModel('gemini-2.0-flash')
        response = model.generate_content("Responde solo con 'OK' si me entiendes")
