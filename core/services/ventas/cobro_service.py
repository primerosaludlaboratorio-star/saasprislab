"""
Servicio de dominio PDV Farmacia (v8.5 Fase 2): cobro atómico.
transaction.atomic vive en ejecutar_venta_pdv; las vistas solo delegan.
Catálogo y resolución de entidades → CatalogoService.
Devoluciones/cancelaciones → DevolucionService.
"""
import logging
...
class VentaFarmaciaService:
    ...
    # ── Delegates to CatalogoService (kept for backward compat with call sites) ──

    @staticmethod
    def materializar_lote_operativo_si_falta(producto, empresa):
        return CatalogoService.materializar_lote_operativo_si_falta(producto, empresa)

    @staticmethod
    def resolver_sucursal_operativa(usuario, empresa):
        return CatalogoService.resolver_sucursal_operativa(usuario, empresa)
