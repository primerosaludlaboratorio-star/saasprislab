...
class MonitorProduccionWorkflowTest(TestCase):
    ...
    def test_avanzar_validado_parcial_bloquea_sin_resultado_lims(self):
        """La aprobación no puede liberar una orden sin valor por analito."""
        ...
        self.assertEqual(data['codigo'], 'RESULTADOS_INCOMPLETOS')
        self.assertIn(self.analito.nombre, data['mensaje'])

        orden.refresh_from_db()
        self.assertEqual(orden.estado_clinico, 'VALIDADO_PARCIAL')

    def test_avanzar_validado_parcial_a_completo_con_resultado_lims(self):
        """El Kanban permite liberar cuando el resultado LIMS ya está capturado."""
        orden = self._crear_orden_lims(estado_clinico='VALIDADO_PARCIAL')
