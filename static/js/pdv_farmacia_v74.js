...
function _pdvInyectarHtmlFragmento(html, cont, sp, opts) {
    ...
    if (typeof window._pdvHacerVisibleResultados === 'function') {
...
window.initPdvResultados = function() {
    ...
    cont.addEventListener('click', function(ev) {
        ...
        if (pid) window.intentarAgregar(pid);
    });
    ...
    cont.dataset.pdvResultsBound = '1';
    return true;
};
...
window._pdvHacerVisibleResultados = function() {
    var cont = document.getElementById('search-results-container');
    if (!cont) return;
    cont.scrollTop = 0;
};
function _renderResultados(productos, termino) {
    ...
    var html = '<div class="list-group list-group-flush p-2">';
    productos.forEach(function(p) {
        ...
        html += '<button type="button" class="list-group-item list-group-item-action d-flex align-items-start justify-content-between gap-3 pdv-result-item ' + (bloqueado ? 'opacity-50 border-danger' : '') + '" data-producto-id="' + p.id + '" aria-label="' + aria.replace(/"/g, '&quot;') + '" ' + (bloqueado ? 'disabled' : '') + ' style="text-align:left;cursor:' + (bloqueado ? 'not-allowed' : 'pointer') + '">';
        ...
        html += '</div>';
        html += '<div class="text-end flex-shrink-0"><div class="text-primary fw-bold fs-5">'+_fmt(p.precio_base)+'</div></div>';
        html += '</button>';
    });
    cont.innerHTML = html + '</div>';
    // El navegador conserva el scroll anterior al reemplazar el HTML; forzamos
    // el inicio después del render para que el primer resultado quede visible.
    cont.scrollTop = 0;
    window._pdvHacerVisibleResultados();
}
...
function _seleccionarLotePDV(producto, lotes) {
    return new Promise(function(resolve) {
        ...
        modal.className = 'modal fade show'; modal.style.display = 'block'; modal.style.background = 'rgba(0,0,0,.55)'; modal.setAttribute('role', 'dialog');
        var dialog = document.createElement('div'); dialog.className = 'modal-dialog modal-dialog-centered';
        var content = document.createElement('div'); content.className = 'modal-content';
        var header = document.createElement('div'); header.className = 'modal-header bg-primary text-white'; header.innerHTML = '<h5 class="modal-title"><i class="bi bi-box-seam me-2"></i>Seleccionar lote</h5>';
        var body = document.createElement('div'); body.className = 'modal-body';
        var title = document.createElement('p'); title.className = 'fw-bold mb-3'; title.textContent = producto.nombre_comercial || 'Producto'; body.appendChild(title);
        var list = document.createElement('div'); list.className = 'list-group';
        lotes.forEach(function(lote) { var button = document.createElement('button'); button.type = 'button'; button.className = 'list-group-item list-group-item-action text-start'; button.innerHTML = '<strong>' + (lote.numero_lote || 'Sin lote') + '</strong><br><small>Caducidad: ' + (lote.fecha_caducidad || 'N/A') + ' | Existencia: ' + (lote.cantidad || 0) + '</small>'; button.addEventListener('click', function() { modal.remove(); resolve(lote); }); list.appendChild(button); });
        body.appendChild(list);
        var footer = document.createElement('div'); footer.className = 'modal-footer'; var cancel = document.createElement('button'); cancel.type = 'button'; cancel.className = 'btn btn-outline-secondary'; cancel.textContent = 'Cancelar'; cancel.addEventListener('click', function() { modal.remove(); resolve(null); }); footer.appendChild(cancel);
...
window.intentarAgregar = function(productoId) {
    ...
    .then(function(data){
        ...
        if (lotes.length > 1) { _seleccionarLotePDV(prod, lotes).then(function(lote) { if (!lote) return; _procesarProductoPDV(Object.assign({}, prod, {lote_id:lote.id, numero_lote_proximo:lote.numero_lote, proxima_caducidad:lote.fecha_caducidad, dias_restantes:lote.dias_restantes, stock:lote.cantidad, stock_total:lote.cantidad, costo_lote:lote.costo_adquisicion})); }); return; }
