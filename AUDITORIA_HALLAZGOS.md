...
- **Recomendación (prioridad máxima):**
  1. Agregar `get_queryset()` con filtro por `request.user.empresa_id` a TODAS las `ModelAdmin` de datos de negocio en `core/admin/`, replicando el patrón ya usado en `identidad.py`.
  2. Habilitar `ADMIN_GROUP_RESTRICTION_ENABLED=True` en producción como mínimo indispensable mientras se corrige el punto 1.
  3. Reconsiderar si el DIRECTOR de un tenant realmente necesita `is_staff=True` (acceso a `/admin/`) en el flujo de onboarding, o si debería gestionarse todo desde las vistas de negocio con `role_required`.
- **Estado:** pendiente de decisión del usuario. Este es, hasta el momento, el hallazgo de mayor severidad de todo el Bloque 2.
