...
- **Estado:** corregido en runtime: el alias público apunta a `core.ConsultaMedica`, `Somatometria` ya no se registra como modelo activo ni aparece en Admin, y no quedan imports operativos al modelo legacy. Las tablas históricas se conservan sin acceso operativo para no destruir evidencia; su eliminación física queda reservada a una migración mayor con inventario y aprobación de retención.
