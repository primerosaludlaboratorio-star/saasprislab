...
## H-NUEVO-10 — Contrato RBAC duplicado entre guards HTTP y guards de servicio — CERRADO COMO ARQUITECTURA CONTROLADA
...
- **Cierre:** `core.decorators.role_required` es el único guard obligatorio para vistas HTTP. `core.rbac.permissions` queda reservado a servicios, control de sucursal y consumidores explícitos; sus decoradores no se presentan como enforcement automático de endpoints. `PERMISSION_MAP` conserva la política consultable para `check_permission`/`user_permissions`, y las pruebas de `core.rbac.tests` cubren whitelist estricta, denegación por rol, superusuario y sucursal.
- **Estado:** cerrado como deuda controlada; no se duplica lógica ni se elimina la API porque tiene consumidores y pruebas activos.
...
## H-NUEVO-58 — Autoaprobación sin control de rol en flujos de aprobación interna (`ValeRequisicion` y `OrdenDeCompra`) — ALTO, CORREGIDO Y PROBADO
...
- **Verificación actual:** ambas ramas contienen control de rol (`ADMIN`, `GERENTE`, `DIRECTOR`) y rechazo explícito del solicitante como aprobador. La prueba `inventario.tests.test_inventario.ValeRequisicionTest` terminó sin error visible; `manage.py check` se mantiene correcto.
- **Estado:** cerrado en código y verificado; se incluye en el despliegue vigente.
...
## H-NUEVO-62 — Regeneración y lectura de códigos de respaldo 2FA sin reautenticación, con almacenamiento recuperable — ALTO, CORREGIDO
...
- **Descripción:** el flujo anterior permitía consultar los códigos con un `GET` autenticado y regenerarlos sin reautenticación. Esto queda corregido: la pantalla requiere una marca de sesión de generación reciente y la consume al primer acceso; la regeneración exige la contraseña actual; el admin excluye el campo; `__str__` no revela el código; los códigos nuevos usan hash adaptativo y los legacy se rehashean al validarse.
- **Riesgo residual controlado:** el código se muestra una sola vez al usuario que acaba de activar/regenerar 2FA. Si se pierde, se debe regenerar mediante reautenticación; no existe una ruta de recuperación silenciosa.
- **Evidencia:** `seguridad/tests.py::TwoFactorTest.test_codigos_backup_no_se_pueden_consultar_dos_veces` y `test_regenerar_codigos_exige_reautenticacion`; migración `seguridad.0006`; `CodigoBackup2FAAdmin` excluye `codigo`.
...
## H-NUEVO-66 — `bienestar` almacena el diario emocional en texto plano, sin campo `empresa` ni cifrado real, y expone recursos sin aislamiento de tenant — MEDIO, CORREGIDO Y DESPLEGADO
...
- **Corrección aplicada:** `DiarioEmocional.contenido_privado` usa `EncryptedTextField` fail-closed; se añadió `empresa` con backfill desde `usuario.empresa`, validación contra reasignación cross-tenant y consultas personales explícitamente scoped. `RecursoCrecimiento` distingue recursos del tenant de recursos globales; las vistas y el Admin rechazan recursos privados de otra empresa y no permiten editar recursos globales desde un tenant. El Admin nunca descifra ni muestra el contenido emocional.
- **Migración:** `bienestar.0004_privacidad_tenant_diario` cifró históricos durante la migración y recuperó la empresa cuando el usuario la tenía asignada.
- **Pruebas:** `core.tests.test_bienestar_privacy` — 3 pruebas OK; `manage.py check`, `makemigrations --check --dry-run` y migración local completa OK.
- **Estado:** corregido, probado y desplegado en producción. La suite heredada `bienestar.tests` no terminó dentro del límite del runner local por su inicialización completa; no se presenta como evidencia adicional de cierre.
...
## H-NUEVO-73 — APIs de IA/transcripción en `consultorio` permiten sobrescribir la transcripción de cualquier consulta y envían datos a Gemini sin rate limiting ni validación del output — MEDIO/ALTO, CORREGIDO Y DESPLEGADO
...
- **Corrección aplicada:** `api_analizar_transcripcion` exige que un usuario con rol `MEDICO` sea el médico asignado a la cita antes de persistir la transcripción; administración/director mantienen la operación administrativa dentro de su empresa. Se añadió rate limit específico a transcripción y procesamiento de audio. La respuesta SOAP se reduce a un contrato allowlist, valida tipos, limita tamaños, limita medicamentos y normaliza signos antes de cualquier persistencia. El servicio de audio ya valida y limpia su respuesta estructurada.
- **Pruebas:** `core.tests.test_consultorio_ai_security` — 2 pruebas OK: bloqueo de escritura sobre consulta ajena y eliminación de campos no permitidos. `manage.py check` y compilación dirigida OK.
- **Estado:** corregido, probado y desplegado en producción en `8a520dc7d18a210be3297ec8f425e8e16a1c31cf`; health check 200 con base de datos y caché operativas.
...
## H-NUEVO-76 — `consultorio` mantiene modelos legacy (`ConsultaMedica` en `legacy.py`, `Somatometria` sin `empresa`) que confunden el modelo de datos activo y rompen el aislamiento — BAJO/MEDIO, CORREGIDO EN RUNTIME
...
- **Corrección aplicada:** la búsqueda completa no encontró consumidores activos fuera de migraciones y del comando explícito de limpieza. Se retiraron `ConsultaMedicaLegacyAdmin` y `SomatometriaAdmin`; ambos modelos ya no se reexportan desde `consultorio.models`, por lo que no forman parte del runtime ni del Admin operativo. El flujo activo usa `core.ConsultaMedica` y `core.SignosVitales`. No se eliminaron tablas históricas ni datos; los módulos legacy se conservan únicamente para compatibilidad de migraciones/limpieza y quedan fuera de nuevos desarrollos.
- **Estado:** corregido y desplegado: el alias público apunta a `core.ConsultaMedica`, `Somatometria` ya no se registra como modelo activo ni aparece en Admin, y no quedan imports operativos al modelo legacy. La migración `consultorio.0010_quarantine_legacy_models` retira ambos modelos del estado Django sin borrar tablas históricas; la eliminación física queda reservada a una migración mayor con inventario y aprobación de retención.
...
- **Estado:** corregido en el flujo activo y desplegado en producción en `5ab5afa1e1208536a554bda7c380d397294d8500`; health check 200 con base de datos y caché operativas.
...
- **Severidad**: Media.
- **Hallazgo**: `buzon_kanban` (línea 91) exige `@role_required('DIRECTOR', 'ADMIN', 'GERENTE')` para ver el panel Kanban de quejas. Sin embargo, las dos APIs que ese panel consume — `api_cambiar_estado_queja` (línea 162) y `api_obtener_quejas` (línea 220) — solo tienen `@login_required`, sin `@role_required`. Como son endpoints HTTP independientes (no dependen de la sesión de la vista Kanban para autorizar), cualquier empleado autenticado del tenant, sin rol de dirección, puede: (1) vía `api_obtener_quejas`, leer todas las quejas de la empresa incluyendo `nombre_remitente` y `contacto` cuando `anonimo=False` (identidad de quien reportó, potencialmente contra ese mismo empleado o un compañero); (2) vía `api_cambiar_estado_queja`, cambiar el estado de cualquier queja a `RESUELTO`/`DESCARTADO` y escribir `notas_resolucion`/`notas_seguimiento`, permitiendo ocultar o cerrar reportes sin autorización de RRHH/Dirección.
- **Riesgo**: Fuga de identidad de denunciantes (chilling effect sobre el canal de quejas) y manipulación/encubrimiento de reportes internos por personal sin autoridad, dentro del mismo tenant.
...
### H-NUEVO-152: `catalogo_medicos` podía secuestrar/mutar un registro `Medico` de otro tenant mediante `get_or_create` sin filtrar por `empresa` — CORREGIDO
- **Archivo**: `core/views/catalogos.py`.
- **Líneas**: 46-83.
- **Severidad**: Alta.
- **Hallazgo**: `Medico` (`core/models/catalogos.py:280-290`) tiene `UniqueConstraint(fields=['empresa', 'cedula_profesional'], name='unique_empresa_cedula_medico')`, es decir, **por diseño distintos tenants pueden tener cada uno su propio médico con la misma cédula profesional**. Sin embargo, `catalogo_medicos` hace `Medico.objects.get_or_create(cedula_profesional=cedula, defaults={'nombre_completo': nombre, 'especialidad': especialidad, 'empresa': empresa})` — la búsqueda **no incluye `empresa` en el lookup**. Si un médico con esa cédula ya existe para **otro tenant**, `get_or_create` lo encuentra (`creado=False`) y el código a continuación ejecuta incondicionalmente `medico.nombre_completo = nombre; medico.especialidad = especialidad; if empresa: medico.empresa = empresa; medico.save()` — esto **reasigna el registro del médico de otro tenant a la empresa del usuario actual**, sobrescribiendo su nombre y especialidad, y el tenant original lo pierde de su catálogo (`catalogo_medicos` filtra por `empresa=empresa`, así que desaparece de su lista). Si ambos tenants ya tuvieran su propio médico con esa misma cédula (posible por el `UniqueConstraint` compuesto), la falta de filtro por `empresa` en el `get()` interno de `get_or_create` provocaría `Medico.MultipleObjectsReturned`, capturado por el `except Exception` genérico como error 500.
- **Riesgo**: Secuestro cross-tenant de un catálogo médico (nombre, especialidad) mediante la simple coincidencia de una cédula profesional (dato público/adivinable), permitiendo a un usuario de un tenant corromper o "robar" el registro de un médico usado por otro tenant en órdenes de servicio (`OrdenDeServicio.medico`), potencialmente afectando la trazabilidad de órdenes previas del tenant víctima.
- **Recomendación**: Cambiar el lookup a `Medico.objects.get_or_create(cedula_profesional=cedula, empresa=empresa, defaults={'nombre_completo': nombre, 'especialidad': especialidad})`, alineando la búsqueda con el `UniqueConstraint` real del modelo.

### H-NUEVO-153: `historial_resultados.py` consulta el catálogo `lims.Analito` sin filtrar por `empresa` pese a ser un `TenantModel` obligatorio
- **Archivo**: `core/views/historial_resultados.py`.
- **Líneas**: 38 (`historial_resultados`), 64 (`historial_resultados`, gráfica embebida), 123 (`api_resultados_grafica`), 176 (`comparar_resultados`).
- **Severidad**: Media.
...
- **Severidad**: Baja (funcional) / Media (si se corrige el import sin agregar el filtro de tenant).
...
### H-NUEVO-158 (ALTO): Portal del Paciente (`/pacientes/portal/`) sin rate-limiting en login — fuerza bruta sin mitigación contra cuentas de pacientes reales
- **Archivo**: `pacientes/portal_views.py:31-57` (`portal_login`); configuración de límites en `core/middleware/rate_limit.py:25-37` (`RATE_LIMITS`).
- **Hallazgo**: `RateLimitMiddleware.RATE_LIMITS` protege explícitamente `/login/`, `/accounts/login/`, `/admin/login/`, `/crear-admin-rescate/`, `/ingreso-magico/` y `/contabilidad/api/autofactura/generar/` (5 intentos/5 min), pero **no incluye `/pacientes/portal/`** (confirmado el mount en `config/urls/modulos.py:212`: `path('pacientes/', include(('pacientes.urls', 'pacientes')))`). `portal_login` autentica manualmente contra `UsuarioPaciente.check_password()` sin ningún decorador de límite de tasa ni bloqueo de cuenta tras intentos fallidos. Un atacante puede ejecutar fuerza bruta/credential-stuffing ilimitado contra cuentas reales de pacientes (que dan acceso a resultados de laboratorio, recetas y notas de consulta) sin ninguna fricción, a diferencia del login de staff que sí está protegido.
- **Riesgo**: Compromiso de cuentas de pacientes vía fuerza bruta/diccionario, exponiendo expedientes clínicos completos (resultados de laboratorio, recetas, consultas) del paciente afectado.
- **Recomendación**: Añadir `/pacientes/portal/` (y idealmente `/pacientes/portal/cambiar-password/`) a `RateLimitMiddleware.RATE_LIMITS`, o aplicar `core.decorators.rate_limit` directamente sobre `portal_login`. Considerar además un contador de intentos fallidos por cuenta (no solo por IP) para mitigar spraying distribuido.
- **Corrección aplicada**: se añadió `/pacientes/portal/` al límite de `RateLimitMiddleware` con un máximo de 5 intentos por IP cada 5 minutos. El límite se ejecuta antes de la vista y conserva la respuesta 429 uniforme del middleware.
- **Verificación**: prueba focalizada `core.tests.test_patient_portal_security.test_portal_login_is_rate_limited`, más `manage.py check`, compilación y `makemigrations --check`.
- **Estado**: corregido localmente; pendiente de despliegue junto con la migración del bloque de pacientes.

### H-NUEVO-159 (MEDIO): Flujo de aprobación de `SolicitudAccesoPortal` inoperante para administradores de tenant vía Django Admin, con FK `paciente` sin acotar por tenant
- **Archivo**: `pacientes/admin.py:74-139` (`SolicitudAccesoPortalAdmin`); `core/admin/tenant.py:28-51` (`TenantScopedAdminMixin.get_queryset`); `pacientes/portal_models.py:84-138` (`SolicitudAccesoPortal`).
- **Hallazgo**: `SolicitudAccesoPortal` no tiene campo `empresa` propio; su único vínculo de tenant es a través de `paciente` (FK nullable, se asigna solo AL aprobar) o `respondido_por` (FK nullable, se llena solo al responder). `TenantScopedAdminMixin.get_queryset` prueba las rutas de tenant descubiertas dinámicamente y usa la **primera que compile** (`paciente__empresa_id`), sin verificar si realmente devuelve filas. Para una solicitud recién creada desde el formulario público (`solicitar_acceso`), ambos campos son `NULL`, así que el filtro `paciente__empresa_id=<empresa del admin>` excluye la fila por completo (JOIN interno sobre FK nula) — **ningún administrador de tenant (no-superuser) puede ver las solicitudes de acceso al portal pendientes de revisión**, que es precisamente el estado en que más se necesita su intervención. Solo un `is_superuser` (bypass total) puede verlas y, al hacerlo, el campo `paciente` se presenta como un `<select>` de Django Admin sin `formfield_for_foreignkey` que lo acote por tenant, permitiendo enlazar por error (o de forma maliciosa) la solicitud a un `Paciente` de **otra empresa**, creando una cuenta de portal (`UsuarioPaciente`) con acceso a datos clínicos de un paciente ajeno al tenant que originó la solicitud.
- **Riesgo**: (1) Bloqueo funcional del flujo de alta al portal de pacientes para todo tenant sin superuser. (2) Si se usa la vía superuser, ausencia de control que impida vincular una solicitud a un paciente de un tenant distinto, lo que crearía una cuenta de portal con acceso cross-tenant a expedientes clínicos.
- **Recomendación**: Agregar un campo `empresa` explícito a `SolicitudAccesoPortal` (poblado en `solicitar_acceso` si se puede resolver el tenant público, o dejarlo pendiente de triage por superuser) y usarlo como base del scoping del admin en lugar de depender de FKs que están vacías en el estado más común. Sobrescribir `formfield_for_foreignkey` en `SolicitudAccesoPortalAdmin` para acotar el `<select>` de `paciente` a la empresa del admin autenticado.

### H-NUEVO-160 (MEDIO/ALTO): `recepcion/views.py::cobrar_consulta` cobra dinero sin crear ningún registro financiero (sin `Pago`/`MovimientoCaja`/`CobroConsulta`)
- **Archivo**: `recepcion/views.py:246-289` (`cobrar_consulta`).
- **Hallazgo**: La vista recibe `monto` y `metodo_pago` por POST, los valida (monto no negativo, decimal), y **solo cambia `CitaMedica.estado` a `'COMPLETADA'`** dentro de una transacción con `select_for_update()`. El propio comentario del código lo reconoce: `# metodo_pago recibido por contrato UI; persistencia en módulo de cobros cuando exista modelo`. No se crea ningún `Pago`, `MovimientoCaja`, `CobroConsulta` (existe en `consultorio/models/cobros.py` para el mismo caso de uso) ni entrada de auditoría — el único rastro es un mensaje flash de Django que desaparece tras la siguiente petición. Cualquier usuario autenticado (solo `@login_required`, sin `role_required`) puede marcar cualquier cita de su empresa como pagada por cualquier monto sin que quede ningún registro financiero verificable, y el sistema de corte de caja/arqueo no tiene forma de detectar ni reconciliar ese cobro.
- **Riesgo**: Pérdida de trazabilidad financiera en el punto de cobro de recepción — abre la puerta a cobrar en efectivo sin declarar el ingreso (fuga de caja) sin dejar evidencia alguna, y a discrepancias no auditables entre lo cobrado y lo reportado en cierres de caja.
- **Recomendación**: Persistir el cobro en el modelo de cobros vigente (p. ej. `consultorio/models/cobros.py::CobroConsulta` o el análogo aplicable), vinculado a `CitaMedica`, `empresa`, `sucursal`, usuario cajero, monto, método de pago y con integración al corte de caja/arqueo existente en el resto del sistema (patrón ya usado en `farmacia/views/caja.py` y `consultorio/models/cobros.py::CajaConsultorio`).

### H-NUEVO-161 (CRÍTICO): `suscripciones/views.py::lista_suscripciones` filtra por `is_staff` en vez de tenant — fuga de datos de facturación (Stripe) de TODOS los tenants a cualquier empleado
- **Archivo**: `suscripciones/views.py:12-18` (`lista_suscripciones`); ruta activa confirmada en `config/urls/modulos.py:26` (`path('suscripciones/', suscripciones_views.lista_suscripciones, ...)`, sin prefijo de superadmin).
- **Hallazgo**: La vista solo exige `@login_required` + `if not request.user.is_staff: return redirect(...)`. `is_staff` **no es exclusivo de superusuarios de plataforma**: se confirma en `core/management/commands/crear_usuarios_produccion.py` que se asigna `is_staff=True` a roles operativos normales de un tenant (`DIRECTOR`, `QUIMICO`), es decir, cualquier usuario con acceso al Django Admin de su propia empresa. La vista no filtra por `empresa` en ningún punto: `SuscripcionTenant.objects.select_related('empresa', 'plan').order_by('-fecha_inicio')` devuelve **las suscripciones de TODAS las empresas de la plataforma**, incluyendo `stripe_customer_id`, `stripe_subscription_id`, `estado` (activa/vencida/cancelada) y `plan` de cada tenant, renderizadas en `suscripciones/lista.html` para cualquier empleado con `is_staff=True` de cualquier tenant.
- **Riesgo**: Fuga de datos de facturación de todos los clientes de la plataforma SaaS (identificadores de Stripe, estado de pago, plan contratado) a personal operativo de cualquier tenant — no solo un problema de aislamiento entre clínicas, sino exposición de información comercial/financiera sensible de la propia operadora del SaaS frente a sus clientes.
- **Recomendación**: Restringir con `user.is_superuser` (o un rol explícito de "operador de plataforma", distinto de `is_staff` de tenant) en lugar de `is_staff`; alternativamente, mover la vista bajo `/admin/` con `TenantScopedAdmin`/superuser-only, o filtrar explícitamente por la empresa del usuario si el caso de uso es que cada tenant vea solo su propia suscripción.
- **Corrección aplicada**: la vista exige empresa asignada y filtra `SuscripcionTenant` por `empresa=request.user.empresa`. Usuarios sin empresa reciben `403`.
- **Estado**: corregido localmente; pendiente de despliegue.

### H-NUEVO-162 (CRÍTICO): APIs públicas del Kiosko IoT usan un token compartido para TODA la plataforma y no acotan `Kiosco`/`VerificacionKiosco` por empresa — IDOR cross-tenant cuando el kiosko no tiene IP fija configurada
- **Archivo**: `iot/views.py:89-171` (`api_kiosco_heartbeat`, `api_kiosco_confirmar`, `api_kiosco_rechazar`); `iot/views_api.py:10-55` (`api_kiosco_checkin`); `core/decorators.py:47-72` (`require_api_token`); modelo `iot/models.py:28-32` (`Kiosco.ip_address`, opcional).
- **Hallazgo**: Estos 4 endpoints públicos (`@csrf_exempt`, sin sesión) se protegen únicamente con `@require_api_token('PRISLAB_KIOSCO_API_TOKEN')`, que es **un único secreto estático compartido por TODA la plataforma** (una sola variable de entorno, no hay token por tenant ni por kiosko individual — confirmado en `core/decorators.py:59-63`, que solo compara contra el valor de la variable de entorno). Los lookups de datos (`Kiosco.objects.get(id=kiosco_id, activo=True)` en `views.py:96` y `views_api.py:19`; `VerificacionKiosco.objects.get(id=verificacion_id, estado='PENDIENTE', ...)` en `views.py:131` y `:158`) **no filtran por `empresa` en absoluto** — solo por PK entero secuencial. La única mitigación adicional es una comparación de IP contra `Kiosco.ip_address`, pero ese campo es **opcional** (`blank=True, null=True`) y el propio código lo trata como condicional: `if kiosco.ip_address: ... else:` (sin control alguno si no está configurado). Los tests existentes (`iot/tests.py::test_api_kiosco_*_ip_whitelist`) solo cubren el caso donde `ip_address` SÍ está configurada; no existe ninguna prueba de aislamiento cross-tenant por `kiosco_id`/`verificacion_id` (a diferencia de `test_api_toggle_kiosco_cross_tenant`/`test_api_enviar_a_kiosco_cross_tenant`, que sí prueban esto para los endpoints con sesión).
- **Riesgo**: Cualquiera que obtenga el único token compartido (p. ej. extrayéndolo de un solo dispositivo kiosko físico comprometido, o de una app/config filtrada) puede, para cualquier kiosko sin IP fija configurada de **cualquier tenant**: (1) leer datos de pacientes en verificaciones pendientes vía heartbeat (`datos_mostrar` incluye nombre y estudios), (2) confirmar/rechazar verificaciones de check-in de pacientes de otro tenant, y (3) en `api_kiosco_checkin`, marcar `OrdenDeServicio.estado = 'EN_PROCESO'` y crear registros `VerificacionKiosco` falsos para órdenes de otro tenant, simplemente enumerando IDs enteros secuenciales de `kiosco_id`/`orden_id`.
- **Recomendación**: Emitir un token/secreto único por kiosko (o al menos por tenant) en vez de un único valor de plataforma; hacer `Kiosco.ip_address` obligatorio o, si debe ser opcional, exigir un token por kiosko como sustituto de la verificación de IP; y acotar explícitamente todos los `get()`/`filter()` de `Kiosco`/`VerificacionKiosco` en estos endpoints por la relación de tenant disponible en el propio objeto (p. ej. cruzando `orden.empresa` contra `kiosco.empresa` antes de proceder), no solo por PK.
- **Corrección aplicada**: cada kiosco ahora tiene `api_token_hash` propio; heartbeat, confirmación, rechazo y check-in validan la credencial del kiosco objetivo. También se corrigió el vínculo `orden_id` del check-in.
- **Estado**: corregido localmente; requiere reprovisionar kioscos existentes antes del despliegue operativo.

### H-NUEVO-163 (CRÍTICO): `logistica/views.py` es un segundo sistema de "Transferencias entre Sucursales" en producción, sin ninguna de las protecciones RBAC/concurrencia ya aplicadas al módulo canónico `core/views/transferencias.py`
- **Archivo**: `logistica/views.py:128-445` (`crear_transferencia`, `enviar_transferencia`, `recibir_transferencia`); modelo `logistica/models.py:65-183` (`TransferenciaInventario`, distinto del usado por `core/views/transferencias.py`). Ruta activa: `path('logistica/', include(('logistica.urls', 'logistica')))` en `config/urls/modulos.py:213` → `logistica/transferencias/...`.
- **Hallazgo**: Existe un módulo **paralelo y duplicado** al ya corregido `core/views/transferencias.py` (protegido con `role_required`/`permission_required` + `select_for_update()`, verificado por `core/tests/test_transferencias_security.py`, montado en `/transferencias/` vía `config/urls/finanzas.py`). Este segundo módulo, montado en `/logistica/transferencias/...`, con su propio modelo `TransferenciaInventario` (mismo nombre de clase, tabla distinta), reimplementa el mismo flujo de negocio (crear → enviar → recibir, con impacto directo en `farmacia.MovimientoInventario` y por tanto en el stock real) pero:
  - `crear_transferencia`: solo `@login_required`, sin `role_required`/`permission_required` — cualquier usuario autenticado de la empresa puede crear transferencias.
  - `recibir_transferencia`: **solo `@login_required`, sin ninguna verificación de rol/permiso** — cualquier usuario autenticado puede completar la recepción, lo cual crea automáticamente dos `MovimientoInventario` (salida en origen, entrada en destino) y actualiza el estado a `COMPLETADA`. **Esto está confirmado empíricamente por el propio test existente** `logistica/tests.py::test_recibir_transferencia_kardex_integration`, que usa un usuario `testuser` sin rol especial (por tanto sin privilegios elevados) y verifica que la recepción se complete exitosamente y genere los movimientos de Kardex — es decir, el propio test de la suite demuestra el bypass sin darse cuenta de que es un hallazgo de seguridad.
  - `enviar_transferencia` sí tiene `@permission_required('logistica.add_transferenciainventario', raise_exception=True)`, pero es un permiso Django genérico de "añadir", no necesariamente restringido a roles gerenciales como en el módulo canónico (`ADMIN`/`DIRECTOR`/`GERENTE`/`FARMACIA`/`QUIMICO`).
  - Ninguna de las tres vistas usa `select_for_update()` sobre `TransferenciaInventario`/`DetalleTransferencia`/`Producto`/`Lote`, a diferencia del módulo canónico — riesgo de condiciones de carrera en recepciones concurrentes de la misma transferencia.
- **Riesgo**: Bypass completo de los controles de segregación de funciones ya implementados para el mismo caso de uso de negocio, a través de una ruta alterna (`/logistica/transferencias/` en vez de `/transferencias/`). Cualquier usuario autenticado de un tenant (incluido personal sin ninguna autorización operativa, p. ej. `CAJERO`) puede mover inventario entre sucursales sin autorización gerencial, y condiciones de carrera pueden duplicar/perder movimientos de Kardex en recepciones simultáneas.
- **Recomendación**: Unificar ambos módulos en una sola implementación canónica (idealmente retirando el modelo/vistas duplicados de `logistica/` o redirigiendo sus URLs al módulo ya corregido), o si deben coexistir, aplicar exactamente el mismo `role_required`/`permission_required` restringido a roles gerenciales y `select_for_update()` que ya tiene `core/views/transferencias.py`.
- **Corrección aplicada**: las cuatro mutaciones del flujo logístico (`crear`, agregar detalle, enviar y recibir) exigen roles operativos autorizados. Enviar y recibir bloquean la transferencia dentro de la transacción; recibir valida cantidades y propaga cualquier error de inventario, evitando marcar como completada una operación incompleta. El folio dejó de usar `count()+1` y ahora usa un sufijo UUID; `total_cantidad()` usa el campo existente `cantidad_solicitada`.
- **Estado**: corregido localmente; pendiente de despliegue.

### H-NUEVO-164 (MEDIO): `ia.TranscripcionVoz` no es `TenantModel` (sin `empresa`) y su panel de Admin agrega estadísticas de TODOS los tenants
- **Archivo**: `ia/models.py:156-214` (`TranscripcionVoz`, hereda de `models.Model` en vez de `core.tenant.TenantModel`, a diferencia de `CotizacionOCR` en el mismo archivo que sí es `TenantModel`); `ia/admin.py:244-264` (`TranscripcionVozAdmin.changelist_view`) y `ia/admin.py:124-144` (`CotizacionOCRAdmin.changelist_view`, mismo patrón pero mitigado — ver abajo).
- **Hallazgo**: `TranscripcionVoz` no tiene campo `empresa` ni usa el manager tenant-aware (`core.tenant.TenantManager`/`TenantQuerySet`, confirmado en `core/tenant.py:268-303` que auto-filtra por `get_current_empresa()`). El listado del Admin sí queda protegido porque `TenantScopedAdminMixin.get_queryset` descubre dinámicamente la ruta `usuario_creador__empresa_id` (Usuario sí tiene `empresa`), pero **`changelist_view` calcula `stats` con `TranscripcionVoz.objects.aggregate(...)` directamente sobre el manager por defecto sin tenant-scoping**, en vez de usar `self.get_queryset(request)`. Esto expone en la página de listado del Admin (visible para cualquier `is_staff=True` de cualquier tenant, ver también H-NUEVO-161 sobre alcance de `is_staff`) el conteo total, confianza promedio y órdenes asociadas de transcripciones de **TODOS los tenants de la plataforma**, no solo del propio. El mismo patrón de `changelist_view` existe en `CotizacionOCRAdmin`, pero ahí `CotizacionOCR.objects` sí es un `TenantManager` auto-scoped, por lo que ese caso está mitigado siempre que el middleware de tenant fije correctamente `get_current_empresa()` durante las peticiones a `/admin/`.
- **Riesgo**: Fuga de métricas agregadas de uso (volumen de transcripciones, confianza promedio, tasa de conversión a órdenes) de otros tenants a personal operativo con acceso a Django Admin — información de negocio de la competencia/otros clientes de la plataforma, aunque sin datos clínicos granulares.
- **Recomendación**: Convertir `TranscripcionVoz` en `TenantModel` con `empresa` explícito (o al menos usar `self.get_queryset(request).aggregate(...)` en `changelist_view` en lugar del manager por defecto), y aplicar el mismo cambio preventivamente en `CotizacionOCRAdmin.changelist_view` para no depender implícitamente de que el middleware de tenant esté activo en el contexto de `/admin/`.
- **Corrección aplicada**: `TranscripcionVozAdmin.changelist_view` ahora calcula las estadísticas con `self.get_queryset(request)`, reutilizando el aislamiento tenant del Admin en lugar del manager global. No se requiere migración.
- **Estado**: corregido localmente; pendiente de despliegue.

### H-NUEVO-165 (MEDIO): `POST /api/caja/corte-unificado/` cierra caja de Farmacia+Laboratorio sin dejar registro en `AuditLog`, a diferencia del endpoint canónico de corte de caja
- **Archivo**: `farmacia/views/corte_caja_api.py:64-133` (`api_corte_caja_unificado`, mapeado en `config/urls/api.py:18` a `/api/caja/corte-unificado/`); `farmacia/services/corte_caja_unificado.py:220-362` (`cerrar_turno_unificado`); comparar con `farmacia/views/caja.py:97-101` (`corte_caja_farmacia`, que sí crea `core.models.AuditLog.objects.create(...)` al cerrar el turno).
...
- **Verificación**: prueba focalizada `core.tests.test_patient_portal_security.test_portal_request_cannot_link_patient_from_other_tenant`, más `manage.py check`, compilación y `makemigrations --check`.
- **Estado**: corregido localmente; pendiente de despliegue junto con la migración del bloque de pacientes.
