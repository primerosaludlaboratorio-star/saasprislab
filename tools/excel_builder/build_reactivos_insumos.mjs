...
const perfilesPorAnalito = new Map();
for (const row of limsPerfilRows) {
  const lista = perfilesPorAnalito.get(row.codigoAnalito) || [];
  lista.push(`${row.abreviaturaExamen}: ${row.descripcionExamen}`);
  perfilesPorAnalito.set(row.codigoAnalito, [...new Set(lista)]);
}
...
function tenant(sheet, range = "A6:A205") {
  // Large prefill rows already carry tenant_id in their source data. Keep
  // the blank template area bounded to avoid expanding sparse ranges.
  sheet.getRange("A6:A205").values = Array.from({ length: 200 }, () => [1]);
}
...
setup(reactivos, "AI", "Captura individual de reactivos", "Una fila por reactivo y lote. Precargada por cada analito del catalogo LIMS; confirmar si requiere reactivo fisico y completar identidad, trazabilidad, equipo y consumos.", reactivoHeaders, [12,18,28,28,20,24,20,20,16,18,16,16,24,20,18,18,24,20,18,28,36,20,28,24,24,20,20,18,22,18,22,18,18,18,24], [], Math.max(205, 5 + limsParametros.length)); tenant(reactivos, `A6:A${5 + Math.max(205, limsParametros.length)}`); listValidation(reactivos, `N6:N${5 + Math.max(205, limsParametros.length)}`, ["Sí", "No"]); listValidation(reactivos, `AF6:AF${5 + Math.max(205, limsParametros.length)}`, ["ADJUNTO", "VERIFICADO", "PENDIENTE", "NO_APLICA", "NO_DISPONIBLE"]); listValidation(reactivos, `AG6:AG${5 + Math.max(205, limsParametros.length)}`, ["ADJUNTO", "PENDIENTE", "NO_APLICA", "NO_DISPONIBLE"]); listValidation(reactivos, `AH6:AH${5 + Math.max(205, limsParametros.length)}`, ["CUARENTENA", "ACTIVO", "BLOQUEADO", "VENCIDO", "AGOTADO"]); reactivos.getRange(`K6:L${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "yyyy-mm-dd"; reactivos.getRange(`S6:S${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "$#,##0.00"; reactivos.getRange(`AA6:AD${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "#,##0.0000";
const reactivoCatalogoRows = limsParametros.map((row, index) => [1, `ANALITO-${index + 1}-${row.Codigo || "SIN-CODIGO"}`, `Reactivo por confirmar: ${row.Descripcion}`, row.Descripcion, "", "", row.Codigo, "PRESENTACION_POR_CONFIRMAR", "UNIDAD", "", "", "", "", "", "", "", "", "", "", (perfilesPorAnalito.get(row.Codigo) || []).join("; "), `Precargado desde Parametros.csv. Es_Producto=${row.Es_Producto || "No"}; validar si este analito requiere reactivo fisico.`, "REACTIVO_POR_CONFIRMAR", row.Codigo, "", row.Metodo, "PENDIENTE_LIGAR", "", "", "", "", "", "PENDIENTE", "PENDIENTE", "PENDIENTE", "PENDIENTE_VALIDAR"]);
reactivos.getRange(`A6:AI${5 + reactivoCatalogoRows.length}`).values = reactivoCatalogoRows;
reactivos.getRange(`A6:AI${5 + reactivoCatalogoRows.length}`).format.fill = C.orange;
...
setup(controlesQc, "AF", "Captura individual de controles de calidad", "Una fila por control, nivel y lote. Precargada por analito para completar el control, nivel, equipo, lote, rango y frecuencia reales.", qcHeaders, [12,18,28,18,24,24,20,24,18,16,18,18,22,14,18,18,20,20,24,20,20,36,20,20,22,22,24,20,18,24,28,18], [], Math.max(205, 5 + limsParametros.length)); tenant(controlesQc, `A6:A${5 + Math.max(205, limsParametros.length)}`); controlesQc.getRange(`J6:J${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "yyyy-mm-dd"; controlesQc.getRange(`AF6:AF${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "yyyy-mm-dd"; listValidation(controlesQc, `R6:R${5 + Math.max(205, limsParametros.length)}`, ["Diario", "Por corrida", "Semanal", "Mensual", "Según procedimiento"]); listValidation(controlesQc, `AB6:AB${5 + Math.max(205, limsParametros.length)}`, ["Sí", "No"]); listValidation(controlesQc, `AC6:AC${5 + Math.max(205, limsParametros.length)}`, ["CUARENTENA", "ACTIVO", "BLOQUEADO", "VENCIDO", "AGOTADO"]);
const controlCatalogoRows = limsParametros.map((row, index) => [1, `CONTROL-${index + 1}-${row.Codigo || "SIN-CODIGO"}`, `Control por confirmar: ${row.Descripcion}`, "NIVEL_POR_CONFIRMAR", row.Descripcion, "", "", "", "", "", row.Tipo_muestra, "", "", row.Unidades, "", "", "", "POR_CONFIRMAR", "", "", "", `Precargado desde Parametros.csv; completar control, nivel, lote, rango y frecuencia para ${row.Codigo}.`, "CONTROL_QC_POR_CONFIRMAR", row.Tipo_muestra, "", "", "", "No", "PENDIENTE", "PENDIENTE_VALIDAR", "", ""]);
controlesQc.getRange(`A6:AF${5 + controlCatalogoRows.length}`).values = controlCatalogoRows;
controlesQc.getRange(`A6:AF${5 + controlCatalogoRows.length}`).format.fill = C.orange;
...
setup(calibradores, "AE", "Captura individual de calibradores", "Una fila por calibrador y lote. Precargada por analito para completar equipo, niveles, trazabilidad, frecuencia y consumo reales.", calibradorHeaders, [12,18,28,24,24,24,20,24,18,16,24,20,18,18,22,14,22,24,20,20,36,22,28,22,22,24,20,18,24,28,18], [], Math.max(205, 5 + limsParametros.length)); tenant(calibradores, `A6:A${5 + Math.max(205, limsParametros.length)}`); calibradores.getRange(`J6:J${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "yyyy-mm-dd"; calibradores.getRange(`AE6:AE${5 + Math.max(205, limsParametros.length)}`).format.numberFormat = "yyyy-mm-dd"; listValidation(calibradores, `Q6:Q${5 + Math.max(205, limsParametros.length)}`, ["Por corrida", "Diario", "Semanal", "Mensual", "Según procedimiento"]); listValidation(calibradores, `AA6:AA${5 + Math.max(205, limsParametros.length)}`, ["Sí", "No"]); listValidation(calibradores, `AB6:AB${5 + Math.max(205, limsParametros.length)}`, ["CUARENTENA", "ACTIVO", "BLOQUEADO", "VENCIDO", "AGOTADO"]);
const calibradorCatalogoRows = limsParametros.map((row, index) => [1, `CAL-${index + 1}-${row.Codigo || "SIN-CODIGO"}`, `Calibrador por confirmar: ${row.Descripcion}`, row.Descripcion, "", row.Metodo, "", "", "", "", "TRAZABILIDAD_POR_CONFIRMAR", "NIVELES_POR_CONFIRMAR", "", "", "", "UNIDAD", "POR_CONFIRMAR", "", "", "", `Precargado desde Parametros.csv; confirmar si aplica calibracion para ${row.Codigo}.`, "CALIBRACION_POR_CONFIRMAR", "", "", "", "", "No", "PENDIENTE", "PENDIENTE_VALIDAR", "", ""]);
calibradores.getRange(`A6:AE${5 + calibradorCatalogoRows.length}`).values = calibradorCatalogoRows;
calibradores.getRange(`A6:AE${5 + calibradorCatalogoRows.length}`).format.fill = C.orange;
...
setup(consumibles, "AD", "Captura individual de consumibles analíticos", "Una fila por consumible y lote. Incluye materiales comunes y recipientes derivados de todos los tipos de muestra del catalogo LIMS.", consumibleHeaders, [12,18,30,22,20,24,20,16,18,16,24,30,18,18,20,18,20,24,20,24,36,22,22,20,22,18,18,24,28,18]); tenant(consumibles); listValidation(consumibles, "Q6:Q205", ["Sí", "No"]); listValidation(consumibles, "Z6:Z205", ["Sí", "No"]); listValidation(consumibles, "AA6:AA205", ["CUARENTENA", "ACTIVO", "BLOQUEADO", "VENCIDO", "AGOTADO"]); consumibles.getRange("J6:J205").format.numberFormat = "yyyy-mm-dd"; consumibles.getRange("AD6:AD205").format.numberFormat = "yyyy-mm-dd";
const tiposMuestra = [...new Set(limsParametros.map(row => (row.Tipo_muestra || "OTRAS").trim()).filter(Boolean))].sort();
const consumibleMuestraRows = tiposMuestra.map((tipo, index) => [1, `RECIPIENTE-${index + 1}-${tipo.replace(/[^A-Za-z0-9]+/g, "-")}`, `Recipiente para muestra: ${tipo}`, "RECIPIENTE_MUESTRA", "", "", "PRESENTACION_POR_CONFIRMAR", "UNIDAD", "", "", "", tipo, "", "", "", "UNIDAD", "No", "", "", "", `Precargado por Tipo_muestra de Parametros.csv: ${tipo}; confirmar recipiente, marca, lote y caducidad.`, "MUESTRA", "", "", "", "Sí", "PENDIENTE", "PENDIENTE_VALIDAR", "", ""]);
if (consumibleMuestraRows.length) {
  consumibles.getRange(`A13:AD${12 + consumibleMuestraRows.length}`).values = consumibleMuestraRows;
  consumibles.getRange(`A13:AD${12 + consumibleMuestraRows.length}`).format.fill = C.orange;
}
...
setup(consumoPrueba, "AD", "Consumo detallado por prueba, analito y equipo", "Una fila por relacion perfil/prueba + analito del catalogo. Completar articulo, equipo, cantidades y lote antes de activar la formula.", consumoPruebaHeaders, [12,28,24,18,20,24,24,24,20,16,16,18,18,22,24,20,18,24,18,12,36,18,20,20,18,28,18,18,24,36], [], Math.max(205, 5 + limsPerfilRows.length)); tenant(consumoPrueba, `A6:A${5 + Math.max(205, limsPerfilRows.length)}`); listValidation(consumoPrueba, `E6:E${5 + Math.max(205, limsPerfilRows.length)}`, ["REACTIVO", "CONTROL_QC", "CALIBRADOR", "CONSUMIBLE", "REFACCION", "INSUMO_GENERAL"]); listValidation(consumoPrueba, `J6:K${5 + Math.max(205, limsPerfilRows.length)}`, ["Sí", "No"]); listValidation(consumoPrueba, `O6:O${5 + Math.max(205, limsPerfilRows.length)}`, ["Sí", "No"]); listValidation(consumoPrueba, `T6:T${5 + Math.max(205, limsPerfilRows.length)}`, ["Sí", "No"]); consumoPrueba.getRange(`S6:S${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "yyyy-mm-dd"; consumoPrueba.getRange(`AA6:AB${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "yyyy-mm-dd"; consumoPrueba.getRange(`L6:L${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "#,##0.0000"; consumoPrueba.getRange(`N6:N${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "#,##0.0000"; consumoPrueba.getRange(`V6:V${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "#,##0.0000"; consumoPrueba.getRange(`Y6:Y${5 + Math.max(205, limsPerfilRows.length)}`).format.numberFormat = "#,##0.0000";
const consumoPerfilRows = limsPerfilRows.map(row => [1, row.descripcionExamen, row.descripcionAnalito, "", "REACTIVO", "", "", "ANALISIS", "REACTIVO_PRINCIPAL", "No", "Sí", "", "", "", "No", "", "", "", "", "No", `Precargado desde Examenes_Perfil.csv: ${row.codigoExamen}/${row.codigoAnalito}; completar articulo, equipo, cantidades y lote.`, "", "", "", "", "", "", "", "PENDIENTE", "PENDIENTE_LIGAR"]);
consumoPrueba.getRange(`A6:AD${5 + consumoPerfilRows.length}`).values = consumoPerfilRows;
consumoPrueba.getRange(`A6:AD${5 + consumoPerfilRows.length}`).format.fill = C.orange;
...
const metodosEquipo = [...new Set(limsParametros.map(row => (row.Metodo || "METODO_POR_CONFIRMAR").trim()).filter(Boolean))].sort();
const equiposMetodoRows = metodosEquipo.map((metodo, index) => [1, `METODO-${index + 1}-CONFIRMAR`, `Equipo para metodo: ${metodo}`, "", "", "", "", "", "EQUIPO_POR_CONFIRMAR", "", "PENDIENTE", metodo, "", "", "", "", "", "", "", "", "", "", "", "", "", "PENDIENTE", "", `Precargado por Metodo de Parametros.csv. Registrar el equipo fisico, marca, modelo, serie, interfaz, pruebas y analitos soportados.`]);
if (equiposMetodoRows.length) {
  equipos.getRange(`A8:AB${7 + equiposMetodoRows.length}`).values = equiposMetodoRows;
  equipos.getRange(`A8:AB${7 + equiposMetodoRows.length}`).format.fill = C.orange;
}
...
const consumoAnalitoRows = limsParametros.map(row => [
  ...
  row.Metodo,
  row.Unidades,
  (perfilesPorAnalito.get(row.Codigo) || []).join("; "),
  "",
  "",
  "",
  "",
