...
- [x] `core/views/director.py` — 472/472. **H-NUEVO-143 — CORREGIDO** (verificado en código: `director_analizadores*` ahora usa `empresa_efectiva_request(request)` y filtra `Equipo`/`CodigoParametroEquipo` por `empresa` en las 5 vistas).
- [x] `core/views/war_room.py` — 658/658. **H-NUEVO-144 — CORREGIDO** (verificado: `AUDITORIA_HALLAZGOS.md` documenta el cierre; ver sección "Verificación de cierre del Bloque 5").
- [x] `core/views/monitor_produccion.py` — 738/738. Sin hallazgos; correctamente tenant-scoped con `select_for_update()`.
- [x] `core/views/pris_jarvis.py` — 891/891. **H-NUEVO-145 — CORREGIDO** (verificado: `api_crear_archivo_raw` ahora rechaza con 403 si no hay empresa).

### Bloque 19C — `rh.py`, `farmacia.py`, `paciente_detalle.py`, `entrega_resultados.py`, `reportes_financieros.py`, `consentimiento_digital.py` — 2026-08-12

- [x] `core/views/rh.py` — 576/576. Sin hallazgos; tenant-scoped con `@role_required` en todas las vistas.
- [x] `core/views/farmacia.py` — 475/475. Wrapper legacy que reexporta desde `farmacia/views/` (app separada); sin hallazgos en este archivo. **Pendiente**: auditar `farmacia/views/pdv.py`, `inventario.py`, `devoluciones.py`, `reportes.py` en un bloque futuro (fuera de `core/views/`).
- [x] `core/views/paciente_detalle.py` — 602/602. Sin hallazgos; `ExpedienteClinicoView.get_queryset()` y `exportar_historial_pdf` acotan `Paciente` por `empresa` antes de agregar sub-consultas.
- [x] `core/views/entrega_resultados.py` — 636/636. Sin hallazgos; portal público usa token firmado (Django `signing`) con `oid`+`eid` atados criptográficamente, candado financiero y verificación LFPDPPP antes de exponer resultados.
- [x] `core/views/reportes_financieros.py` — 609/609. Sin hallazgos; todas las vistas y exports Excel usan `@role_required` + filtro `empresa`.
...
- [x] **H-NUEVO-153 (Media, CORREGIDO Y DESPLEGADO)**: `core/views/historial_resultados.py` exige `empresa` en el catálogo y en los tres lookups de `Analito`; prueba de frontera tenant añadida. Producción verificada con health 200.
...
- [x] `incidencias.py` — 204/204. **H-NUEVO-154 (Alta, CORREGIDO Y DESPLEGADO)**: `marcar_incidencia_revisada` exige empresa y filtra el lookup por `id + empresa`; prueba de frontera tenant añadida. Producción verificada con health 200.
- [x] **H-NUEVO-155 (Alta, CORREGIDO Y DESPLEGADO)**: `core/views/voice.py::historial_comandos` limita el historial de directores a la empresa solicitante y el de usuarios ordinarios a usuario + empresa; prueba de frontera tenant añadida. Producción verificada con health 200.
- [x] **H-NUEVO-87 (Media/Alta, CORREGIDO Y DESPLEGADO)**: `ResponsableSanitario` elimina la unicidad global de cédula y aplica unicidad por `empresa + cedula_profesional`; la rotación de responsable activo permanece limitada por empresa. Migración `laboratorio.0021` aplicada en producción; health 200.
...
- [x] **H-NUEVO-155 (Alta, CORREGIDO Y DESPLEGADO)**: `core/views/voice.py` — `historial_comandos` limita los logs a la empresa solicitante; producción verificada con health 200.
...
- [x] Sin hallazgos: `analizador_quejas.py` (análisis por palabras clave, sin acceso a datos), `sucursal_helpers.py`, `trazabilidad.py`, `saludos.py`, `estandares_industriales.py`, `marketing_tracking.py` (tokens de tracking firmados con `sign_track_token`, incluyen `empresa_id` en payload).
- [x] `corrector_errores.py` — sin hallazgos de seguridad; confirmado sin consumidores en el código (`Get-ChildItem` recursivo no encontró referencias), es un script de desarrollo muerto que reescribe archivos fuente por regex; no se ejecuta en producción.
- [x] **H-NUEVO-157 (Media, ABIERTO)**: `core/utils/notificaciones.py` referencia `Notificacion`/`ConfiguracionNotificaciones` con el import comentado ("pendientes de migración"); toda llamada lanza `NameError`. La tarea Celery Beat `core.tasks.notificaciones_tasks.ejecutar_verificaciones_automaticas_todas_empresas` la invoca para cada `Empresa` y el error queda silenciado por un `try/except` genérico — las alertas de stock bajo/caducidad nunca se generan para ningún tenant. Además `ia_resources.py::_enviar_alerta_cuota` importa `crear_notificacion_sistema` (nombre inexistente; la función real es `crear_notificacion`), por lo que las alertas de cuota de IA al 80%/90% tampoco se envían nunca.
- [x] Sin hallazgos: `deepseek_client.py`, `gemini_client.py`, `gemini_transport.py` (clientes IA usan solo API keys de entorno/BYOK, sin datos de tenant embebidos), `paths.py` (generador de rutas de almacenamiento, validación de path traversal en `validar_nombre_archivo`), `pdf_generator.py` (renderizado WeasyPrint genérico), `ranking.py` (filtra `Venta`/`OrdenDeServicio`/`IncidenciaOperativa`/`BuzonQuejas` por `empresa` del usuario evaluado), `whatsapp_sender.py` (envío vía Twilio/Meta con credenciales de entorno, sin lógica de tenant).
- [x] `rh_utils.py::generar_pdi_automatico` hace `EvaluacionDesempeno.objects...get(id=evaluacion_id)` sin filtrar por `empresa` — verificado que los 3 call sites existentes (`core/views/rh.py:442` con una evaluación recién creada en la misma request, y dos comandos de management de confianza) no permiten inyectar un `evaluacion_id` de otro tenant; no se reporta como hallazgo explotable, pero se deja nota de deuda técnica (falta defensa en profundidad) por si se agrega un nuevo call site expuesto a input de usuario.

**`core/utils/` (42 archivos): CIERRE TOTAL CONFIRMADO.** Todos los archivos fueron leídos línea por línea. Hallazgos abiertos: H-NUEVO-156 (informativo, código inalcanzable), H-NUEVO-157 (Media, tarea Celery Beat rota). H-NUEVO-154/155 confirmados corregidos por sesión concurrente.

**`core/views/` (incluyendo subpaquetes `laboratorio/`, `medico/`, `pris_ia/`): CIERRE TOTAL CONFIRMADO.** Todos los archivos `.py` de nivel superior fueron leídos línea por línea en algún bloque (19A-19J o bloques anteriores 1-18). Los hallazgos 153, 154 y 155 quedaron corregidos, probados y desplegados; permanecen únicamente los hallazgos que siguen marcados abiertos en `AUDITORIA_HALLAZGOS.md`.
