...
- [!] core/services/lims/ (asistente_clinico.py, coherencia_clinica.py, interfaces_lims_service.py, orden_recepcion_service.py, resultados_lims_service.py) — revisión de tenant y transacciones realizada; H-NUEVO-18 corregido localmente y pendiente despliegue documental/código final. El bloque continúa en revisión exhaustiva.
...
- [!] auditoria_api.py / auditoria_campo.py — Hallazgo **H-NUEVO-18**: permiten forjar entradas de `AuditLog` con valores arbitrarios controlados por el cliente, sin validar existencia real del objeto ni contrastar el valor anterior/nuevo contra el estado real. Abierto.
...
Pendiente en Bloque 5: resto de `core/views/` (laboratorio.py 134KB, pris_ia.py 78KB ya parcialmente auditado con H-NUEVO-11, pris_jarvis.py, medico.py, war_room.py, director.py, blindaje_expediente.py vista, excepciones_lab.py, entrega_resultados.py, paciente_detalle.py, reportes_financieros.py, subcarpetas laboratorio/, medico/, pris_ia/, y el resto de archivos medianos/pequeños).
