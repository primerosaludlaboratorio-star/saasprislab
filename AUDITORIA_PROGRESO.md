...
- [x] pris_jarvis.py — COMPLETO (877 líneas). Todas las vistas `@login_required` + tenant scoping `empresa=`. Hallazgo **H-NUEVO-21**: `api_confirmar_accion`/`api_rechazar_accion` no repiten el filtro de rol por `modulo_destino` que sí existe en `lista_acciones_pris` (`QUIMICO`→lab, `CAJERO`/`GERENTE`→farmacia) — cualquier usuario autenticado del tenant puede confirmar una `AccionPRIS` de otro módulo (ej. RECEPCION validando resultados de laboratorio vía `laboratorio.validar_resultado`, marcando `Resultado.validado=True` sin ser químico), adivinando/enumerando `accion_id` (entero secuencial tenant-scoped). Abierto.
...
Pendiente en Bloque 5: `pris_ia.py` (78KB, H-NUEVO-11 corregido en el paquete activo y en el legado), resto de `laboratorio.py` (2508+ líneas restantes no muestreadas línea por línea) y resto de `farmacia.py` (17KB, solo aliases legacy revisados), `reportes_financieros.py`, subcarpetas `laboratorio/`, `medico/`, `pris_ia/`, y el resto de archivos medianos/pequeños (~40 archivos: `analytics.py`, `asistencia.py`, `autenticacion_2fa.py`, `autorizaciones.py`, `biblioteca.py`, `bienestar.py`/`bienestar_mejorado.py`, `capacitacion_rag.py`, `catalogos.py`/`catalogos_maestros.py`, `comunicacion.py`, `configuracion.py`, `consentimientos.py`, `consulta_ordenes.py`, `cotizacion.py`, `dashboard_unificado.py`, `expediente.py`, `historial_resultados.py`, `ia_dashboard.py`, `incidencias.py`, `laboratorio_captura.py`, `laboratorio_config.py`, `laboratorio_reportes.py`, `manual.py`, `maquila.py`, `microbiologia.py`, `monitor_produccion.py`, `notificaciones.py`, `omnisearch.py`, `onboarding.py`, `paciente.py`, `pacientes.py`, `paquetes.py`, `pris_checklist.py`, `ranking.py`, `reporte_friccion.py`, `sucursal_modo_inventario_lab.py`, `tarifas.py`, `transferencias.py`, `voice.py`, `administracion_usuarios.py`, `audio_legal.py`, `ai_brain.py`, `cerebro.py`, `coach.py`, `feature_flags_admin.py`, `general.py`, `impresion.py`, `inventario.py`/`inventario_predictivo.py`, `manual.py`, `monitoring.py`, `omnisearch.py`, `operaciones.py`).
