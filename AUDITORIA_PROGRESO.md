...
- [x] **H-NUEVO-156 (Baja/Media, CORREGIDO LOCALMENTE)**: `lims_tokens_v75.py::_resolver_token` usa `Analito`, `PerfilLims` y `PaqueteLims` reales; todas las resoluciones y relaciones quedan filtradas por `empresa`. Prueba focalizada añadida. Pendiente de despliegue.
...
- [x] **H-NUEVO-157 (Media, CORREGIDO LOCALMENTE)**: `core/utils/notificaciones.py` usa `NotificacionSistema`, adapta tipos y referencias legacy, y conserva el alias `crear_notificacion_sistema` para cuota IA. Las alertas de stock/caducidad y cuota ya tienen ruta de persistencia vigente. Prueba focalizada añadida. Pendiente de despliegue.
