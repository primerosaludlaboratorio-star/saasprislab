...
- [x] **H-NUEVO-156 (Baja/Media, CORREGIDO Y DESPLEGADO)**: `lims_tokens_v75.py::_resolver_token` usa `Analito`, `PerfilLims` y `PaqueteLims` reales; todas las resoluciones y relaciones quedan filtradas por `empresa`. Prueba focalizada añadida. Producción verificada con health 200.
...
- [x] **H-NUEVO-157 (Media, CORREGIDO Y DESPLEGADO)**: `core/utils/notificaciones.py` usa `NotificacionSistema`, adapta tipos y referencias legacy, y conserva el alias `crear_notificacion_sistema` para cuota IA. Las alertas de stock/caducidad y cuota ya tienen ruta de persistencia vigente. Prueba focalizada añadida. Producción verificada con health 200.
